...
- **`Home/`**
  ...
  - `Projects.vue`: Project showcase. Data is split into two arrays — `productionProjects` (Topiqu, V Okamihu, HexCodium, Škola Života — real commercial / shipped work) and `experimentProjects` (Nimblo, FastProject — marked `kind: 'experiment'`). A `timelineEntries` computed flattens both into a single render list with a section-header entry injected between them so the timeline rail and GSAP scrollTrigger keep alternating layout across the whole flow. The header carries the R&D / Hackathons / AI Experiments kicker + disclaimer (`projects.experiments.*` i18n keys, EN/CS) clarifying that those entries are AI-augmented prototypes, not production code. Experiment cards also get a violet `Experiment` badge next to the title and a violet timeline dot. Each card renders grouped tech-stack pills (`frontend` / `backend` / `db` / `misc`) sourced from a per-project `tech` object; group labels are i18n'd via `projects.tech.*`, technology names stay untranslated.
